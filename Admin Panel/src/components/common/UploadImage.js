...
function ImageUpload({ event, onSelect, tabActive, user, post, imageError, product }) {
    ...
    useEffect(() => {
        if (user && event) {
            setPreviewImages([`${tabActive ? user?.image : user?.reported_event?.image}`])
            onSelect(tabActive ? user?.image : user?.reported_event?.image)
        }
        if (post && user) {
            let images = tabActive ? user?.images : user?.reported_post?.images
            let img = images?.map(res => (`${res}`))
            setPreviewImages(img)
            onSelect(img)
        }
        if (user && product) {
            setPreviewImages(tabActive ? user?.images : user?.reported_product?.images)
            onSelect(tabActive ? user?.images : user?.reported_product?.images)
        }
    }, [user])
    ...
    return (
        <Card style={{ maxWidth: '400px', margin: 'auto', textAlign: 'center', backgroundColor: 'transparent', background: 'transparent' }}>
            <CardContent>
                ...
                </label> : event && previewImages.length == 0 ? <label htmlFor="file-upload" style={{ cursor: 'pointer' }}>
                    <input
                        type="file"
                        id="file-upload"
                        accept="image/jpeg, image/png"
                ...
                </label> : post && previewImages.length == 4 ? '' : post ? <label htmlFor="file-upload" style={{ cursor: 'pointer' }}>
                    <input
                        type="file"
                        id="file-upload"
                        accept="image/jpeg, image/png"
                ...
                </label> : product && previewImages.length == 2 ? '' : product && <label htmlFor="file-upload" style={{ cursor: 'pointer' }}>
                    <input
                        type="file"
                        id="file-upload"
                        accept="image/jpeg, image/png"
