...
const ViewChemicalDetails = ({user, goBackAllChemical}) => {
	useEffect(() => {
		if (user !== null) {
			setUserId(user.id)
	...
	return (
		<div className={classes.root}>
			...
			<Typography variant="div" className={classes.backButton} onClick={goBackAllChemical}>
				<ArrowBack /> Back
			</Typography>
			<Divider variant="fullWidth" className={classes.divider} />
			<div className={classes.root1}>
				<ChemicalDetailForm user={user} goBackAllChemical={goBackAllChemical} />
			</div>
		</div>
	)
}

export default ViewChemicalDetails
