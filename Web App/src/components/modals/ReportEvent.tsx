...
import {reportEvent, reportPost, reportProduct} from '../../apis/apis'
...
const ReportEventModel = ({onClose, data, button}: any) => {
	const [reason, setReason] = useState('')

	const reportPostFunction = async () => {
		if (button == 'Report Post') {
			await reportPost(data.id, reason)
			toastSuccess('Post Reported is Reported')
		}

		if (button == 'Report Event') {
			await reportEvent(data.id, reason)
			toastSuccess('Event is Reported')
		}
		if (button == 'Report Event') {
			await reportEvent(data.id, reason)
		}
		if (button == 'Report Product') {
			await reportProduct(data.id, reason)
			toastSuccess('Product  is Reported')
		}
		onClose()
	}
	return (
