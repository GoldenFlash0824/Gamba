...
import {orderReport, reportEvent, reportPost, reportProduct} from '../../apis/apis'
...
const ReportModal = ({onClose, data, button, isOrderReport}: any) => {
	const [reason, setReason] = useState('')
	const _dispatch = useDispatch()

	const reportPostFunction = async () => {
		_dispatch(setIsLoading(true))
		onClose()

		if (isOrderReport) {
			if (data?.order_products?.length) {
				let response: any = await orderReport(data?.order_products, reason)
		...
		if (button == 'Report Post') {
			let response: any = await reportPost(data.id, reason)
		...
		if (button == 'Report Event') {
			let response: any = await reportEvent(data.id, reason)
		...
		if (button == 'Report Product') {
			let response: any = await reportProduct(data.id, reason)
			if (response?.success) {
				toastSuccess(response.message)
			} else {
				toastError(response.message)
