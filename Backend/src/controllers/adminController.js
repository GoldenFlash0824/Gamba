...
const searchPaidOrder = async (req, res) => {
    const response = await adminService.searchPaidOrder(req)
    if (response.status) {
        return responseUtil.successResponse(res, response.message, response.data)
    } else {
        return responseUtil.validationErrorResponse(res, response.message)
...
export {
    ...
    paymentHistoryGamba, userSearchBlock, userSearchDisable, stripeTransferDetails, ManagePaidOrder, searchPaidOrder, addNotes, getOrderNotes, cancelOrder, ManageCancelledOrder
}
