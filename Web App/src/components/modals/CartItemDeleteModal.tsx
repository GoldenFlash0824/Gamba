...
const CartItemDeleteModal = ({onClose, openDeletePostModal, id}: any) => {
			...
			<Modal
				...
				<ModalWrapper>
					...
					<Text fontSize={1.5} lineHeight="1.438" fontWeight={700} color="black_300">
							Delete {openDeletePostModal ? 'Event' : 'Item'}
						</Text>
					</Head>
					<Body>
						<Text type="normal" fontWeight={400} color="black_300">Are you sure you want to delete this {openDeletePostModal ? 'Event from cart?' : 'Item ?'} </Text>
					</Body>
					<Footer>
						<Flexed direction="row" align="center" justify="space-between">
							<Button hasBorder type="clear" label="Close" ifClicked={onClose} />
							<Button type="danger" label="Delete" ifClicked={handleDelete} />
						</Flexed>
					</Footer>
				</ModalWrapper>
			</Modal>
